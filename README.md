...
1. 到 GitHub Releases 下载最新的 `SkinDesk-v*-*.zip`。
2. 右键 ZIP，选择“全部解压”。不要直接在压缩包里运行。
3. 如果电脑没有 Python，先安装 Python 3.10 或更高版本。安装界面必须勾选 `Add Python to PATH`。
4. 双击 `启动倒货台.bat`，不需要打开命令行，也不需要手动输入安装命令。
5. 第一次启动会自动安装所需组件，可能需要几分钟。等待浏览器自动打开即可。
6. 使用期间不要关闭“Skin Desk”黑色窗口；关闭它就会停止程序。

如果 Windows 弹出安全提示，请确认文件来自本项目的 GitHub Release 后，选择“仍要运行”。
...
启动脚本会自动创建运行环境并安装依赖。普通用户不需要理解 `.venv`、`pip` 或 `requirements.txt`；后续再次启动会直接复用，不会重复安装。
